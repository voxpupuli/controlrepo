#
# @summary Configure key from ekohl from GitHubs in the authorized_keys file along with supplemental keys
#
# Configure key from ekohl from GitHubs in the authorized_keys file along with supplemental keys
#
class profiles::ssh_keys::people::ekohl {
  profiles::update_ssh_authorized_keys(['ekohl'])

  ssh_authorized_key {
    default:
      ensure => 'present',
      user   => 'root',
      type   => 'ssh-rsa',
    ;
    'ewoud-rsa-1':
      key => '',
    ;
    'ewoud-rsa-2':
      key => '',
    ;
  }
}
